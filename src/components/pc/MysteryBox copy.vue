<template>
  <div class="homewrapper" :style="sharecardBg">
    ...
    <div class="homeFirst rippless" :style="firstBg">
      <!-- <img src="@/assets/boxtest.png" alt /> -->
    </div>
            ...
            <ul class="topnav-show">
              <li>
                <div class="left_sharecard">
                  <p
                    class="title wow animate__animated animate__lightSpeedInLeft"
                    :style="bindboxtitleBg "
                  >Mystery box</p>
                  <p class="wow animate__animated animate__lightSpeedInLeft">
                    Mystery Box is a fun game in SeaWorld. The explorer can open the Mystery Box to get the
                    elements. Collect 5 elements to get a chance to open the Treasure Box, the Box contains a certain
                    amount of SEA , the opening time is 24:00 every day, the explorer can share the SEA released at
                    that time equally, or can choose to open it at the next time, if no one opens it that day, the SEA in
                    the box will be accumulated to the next round. The Mystery Box is not transferable or tradable, but
                    the elements obtained by opening the Mystery Box are allowed.
                  </p>
                  <!-- <p
                    class="wow animate__animated animate__fadeInDown"
                  >You can open the NFT Treasure at 0:00 every day. All left SEA will accumulate to the next time.</p>-->
                </div>
...
export default {
      ...
      if (ethereum && ethereum.isMetaMask) {
        ...
        if (ethereum.networkVersion == 97) {
          ...
          if (ethereum.selectedAddress) {
            newAccount = ethereum.selectedAddress;
          }
        }
      }
  ...
  mounted() {
    ...
    //检查是否连接 metamask
    this.checkIsConnectMetamask();
    let that = this;
    setTimeout(function() {
      that.checkIsConnectMetamask();
    }, 1000);
    //判断是否安装 metamask
    if (ethereum && ethereum.isMetaMask) {
      //订阅账号修改机制
      ethereum.on("accountsChanged", this.checkIsConnectMetamask);
      //订阅切换网络
      ethereum.on("networkChanged", this.checkIsConnectMetamask);
    }
    //创建Web3
    this.createWeb3Js();
  },
